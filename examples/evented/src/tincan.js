...
export default function(CONFIG, isLocal, authUrl){
  ...
  var TINCAN = {
    doStart: function () {
      console.log('☂ doStart:', arguments)
    },
    storeAttemptState: function () {
      console.log('☂ storeAttemptState:', arguments)
    },
    doComplete: function () {
      console.log('☂ doComplete:', arguments)
    },
    doTracking: function () {
      console.log('☂ doTracking:', arguments)
    },
    doPolled: function () {
      console.log('☂ doPolled:', arguments)
    },
    getPollData: function (pollId, callback) {
      console.log('☂ getPollData:', arguments);
      callback({})
    },
    doActionPlan: function () {
      console.log('☂ doActionPlan:', arguments)
    },
    getActionPlanData: function (planId, callback) {
      console.log('☂ getActionPlanData:', arguments);
      callback({})
    }
  };
  ...
  (function initApi(){
    ...
    bus.on('tincan::start', function () {
      TINCAN.doStart();
    });

    bus.on('tincan::storeAttemptState', function ({ completionState }) {
      TINCAN.storeAttemptState(completionState);
    });

    bus.on('tincan::complete', function () {
      TINCAN.doComplete();
    });
    ...
    bus.on('tincan::polled', function ({ identifier, type, question, response }) {
      TINCAN.doPolled(identifier, type, question, response);
    });

    bus.on('tincan::getPollData', function ({ identifier, callback }) {
      TINCAN.getPollData(identifier, callback);
    });

    bus.on('tincan::actionPlanUpdate', function ({ identifier, responses }) {
      TINCAN.doActionPlan(identifier, responses)
    });

    bus.on('tincan::getActionPlanData', function ({ identifier, callback }) {
    ...
    bus.on('tincan::access_course', function ({ courseName, name }) {
    ...
    bus.on('tincan::leave_course', function ({ courseName, name }) {
    ...
    bus.on('tincan::access_section', function ({ courseName, name, type }) {
      var start = timer.start(slugify(name));
    ...
    bus.on('tincan::leave_section', function ({ courseName, name, type }) {
      var end = timer.stop(slugify(name));
      TINCAN.doTracking('leave_section', {courseName: courseName, name: name, type: type, end: end});
      //console.log('leave_section', name, end);
    });

    bus.on('tincan::start_video', function ({ courseName, src, currentTime, section }) {
    ...
    bus.on('tincan::stop_video', function ({ courseName, src, currentTime, section }) {
    ...
    bus.on('tincan::access_help', function ({ courseName }) {
  ...
  var tincanImplement = function(){
    ...
    const TC_COURSE_NAME = {
      "en-US": CONFIG.activity_name
    };

    //
    // CourseDesc for the activity
    const TC_COURSE_DESC = {
      "en-US": CONFIG.activity_name
    };
...
//            },
    ...
    TINCAN['getPollData'] = function (identifier, callback) {
      ...
      var handlePollData = function (result) {
        ...
        callback({
          ...
          submissions: statements.map(function (statement) {
            return {value: statement['result']['response']}
          })
        });
      };
    ...
    TINCAN['getActionPlanData'] = function (identifier, callback) {
      //NOTE: tincan.getStatements sends registration along with the GET request which scopes to a single user
      tincan.getStatements({
        params: {verb: {id: actionPlanVerb}},
        callback: function (err, result) {
          ...
          var responses = result['statements'].filter(function (statement) {
            return (statement['context']['extensions'] && statement['context']['extensions'][actionPlanIdExtension] === actionPlanId) && thisActor(statement)
          });
